...
decl_storage! {
	trait Store for Module<T: Config> as Dispatchstake {
		...
		// Init get(fn is_init): bool;
		pub DispCount get(fn dispatch_count): u64;
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		#[weight = 10_000]
		fn transfer(_origin, stake: T::AccountId, value: u64) -> DispatchResult {
			let staker = ensure_signed(_origin)?;
			let staker_balance = Self::get_balance(&staker);
			

			// Calculate new balances
			let updated_from_balance = staker_balance.checked_sub(value).ok_or(<Error<T>>::InsufficientFunds)?;
			

			// Write new balances to storageS
			<StakeAccount<T>>::insert(staker,(value,0));
        ...
        #[weight = 10_1000]
        fn showdown_dispatch(_origin,index:u64)->DispatchResult{
            let _sender = ensure_signed(_origin)?;

            let index = <DispCount>::get();
           // let end_block:u64 = <Dispatchs<T>::get()> ;//当前轮结束调度    u64
            // let disp = <Dispatchs<T>>::get(index);
            // let minstake:u64  = disp:2;//最小质押量    u64
            // let max_authority:u64  = 21;//最大的质押节点个数   u64
            // let interval:u64  = 100 ;//单次起拍间隔价格     u64
            // let state:bool = true ;//是否在竞拍状态    bool
